...
type Prize = { prizeRank: number; prizeAmount: number; numberOfWinners: number; prizeDescription?: string }
...
export default function PrizeStructure() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      await api.post(`/admin/contests/${id}/prize-structure`, rows)
  ...
  const update = (i: number, key: keyof Prize, value: string) => {
    setRows((prev) => prev.map((r, idx) => idx === i ? { ...r, [key]: key.includes('Amount') || key.includes('Winners') || key === 'prizeRank' ? Number(value) : value } as Prize : r))
  }
    ...
    <div>
      ...
      <form onSubmit={submit} className="space-y-4">
        <div className="space-y-2">
          {rows.map((r, i) => (
            <div key={i} className="grid grid-cols-1 md:grid-cols-5 gap-2 bg-white border rounded p-3">
